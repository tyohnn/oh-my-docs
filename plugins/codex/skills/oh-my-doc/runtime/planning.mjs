...
/** @deprecated MDX catalog collector — local SSOT is HTML under .omd/dbs */
...
function checkNavigation(contentDirectory, document, problems) {
  ...
  try {
    ...
    const pages =
      typeof meta === 'object' && meta !== null && !Array.isArray(meta) ? meta.pages : undefined;
...
/** @deprecated Prefer validateHtmlPlanning for local SSOT */
export function validatePlanning(contentDirectory) {
  ...
  applyGraphRules(collected.documents, byId, problems);
  return problems;
}

/**
 * Validate local HTML catalogs under `.omd/dbs`.
 * @param {string} dbsRoot
 * @param {ReturnType<typeof loadLocalHtmlIaGraph>} [graph]
 */
export function validateHtmlPlanning(dbsRoot, graph = loadLocalHtmlIaGraph()) {
  const collected = collectHtmlDocuments(dbsRoot, graph);
  const problems = [...collected.problems];
  const byId = new Map();
  ...
  for (const document of collected.documents) {
    ...
    if (!document.title) {
      problems.push(`${document.file}: ${document.kind} document is missing title`);
    }
  }
  ...
  for (const document of collected.documents) {
    for (const [relName, ids] of Object.entries(document.relations ?? {})) {
      const expectedKind = relNameToKind(relName);
      for (const id of ids) {
        if (expectedKind) requireReference(document, id, expectedKind);
        else if (!byId.has(id)) {
          problems.push(`${document.file}: ${relName} reference does not exist — ${id}`);
        }
      }
    }
  }
 ...
 */
function relNameToKind(relName) {
  const map = {
    prd: 'prd',
    prds: 'prd',
    stories: 'story',
...
/**
 * Shared lifecycle / plan graph rules for MDX and HTML docs.
 * @param {Array<{ file: string, kind: string, status?: string, stage?: string, changeType?: string, prd?: string, specs?: string[], stories?: string[], codeAreas?: string[] }>} documents
 * @param {Map<string, any>} byId
 * @param {string[]} problems
 */
function applyGraphRules(documents, byId, problems) {
  const requireReference = (owner, id, kind) => {
    const target = byId.get(id);
    if (!target) {
      problems.push(`${owner.file}: ${kind} reference does not exist — ${id}`);
  ...
  for (const document of documents) {
    ...
    if (document.kind === 'feature') {
      if (document.status && !FEATURE_STATUSES.includes(document.status)) {
        problems.push(`${document.file}: feature status must be draft, active, or done`);
      }
      continue;
    }
    if (document.kind === 'release') {
      if (document.status && !RELEASE_STATUSES.includes(document.status)) {
        problems.push(`${document.file}: release status must be draft, active, or done`);
      }
      continue;
    }
    ...
    const needsProductContext =
      document.changeType === 'product' || document.changeType === 'bugfix';
